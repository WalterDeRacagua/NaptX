...
public class KeyStoreManager {
    ...
    public byte[] firmarMensaje(String alias, byte[] mensaje) throws Exception{
        if (!claveExiste(alias)){
            throw new Exception("No existe clave con alias: "+ alias);
        }

        //Únicamente obtenemos una referencia de la clave privada, pero esta no sale realmente del chip de seguridad.
        KeyStore.PrivateKeyEntry privateKeyEntry = (KeyStore.PrivateKeyEntry) keyStore.getEntry(alias,null);

        PrivateKey privateKey = privateKeyEntry.getPrivateKey();

        //Primero hace el hash del mensaje con SHA256 y después firma con el algoritmo de curva elíptica.
        ...
        //Firma con el estándar de Android.
        byte[] firmaCruda = signature.sign();

        Log.d(TAG, "Mensaje firmado exitosamente");

        return convertirFirmaEthereum(firmaCruda);
    }
